...
import FormEditCard from '@/app/(admin)/components/CourseEditFormCard';
import { useRouter, usePathname } from 'next/navigation';
import getCourseById from '@/helpers/Courses/getCourseById';
import updateCourse from '@/helpers/Courses/updateCourse';
...
const Page = () => {
    ...
    const courseId = id[id.length - 1].replace('id=', '');
    ...
    const fetchData = async () => {
        try {
            ...
            if (response && response.courseById) {
                const data = response.courseById;
                // Ensure tags is a string for the form
                const formattedData = {
                    ...data,
                    tags: Array.isArray(data.tags) ? data.tags.join(', ') : data.tags,
                    // Ensure modules have necessary form fields if they are missing
                    modules: data.modules.map(m => ({
                        ...m,
                        moduleId: m.moduleId || crypto.randomUUID(),
                        durationWeek: m.detailedDuration?.weeks || 0,
                        durationDays: m.detailedDuration?.days || 0,
                        durationHours: m.detailedDuration?.hours || 0,
                        durationMinutes: m.detailedDuration?.minutes || 0,
                    }))
                };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await updateCourse({ ...dataToSubmit, courseId });
    ...
    return (
        <div className="min-h-screen w-full p-4 sm:p-8 font-inter">
            ...
            <div className=" mx-aut shadow-2xl rounded-xl p-6 md:p-10">
                ...
                <FormEditCard formData={formData}
